...
/// @title FranchisedKashiPair
/// @dev This contract allows contract calls to any contract (except BentoBox)
/// from arbitrary callers thus, don't trust calls from this contract in any circumstances.
contract FranchisedKashiPair is ERC20, BoringOwnable, IMasterContract {
    ...
    event LogFeeTo(address indexed newFeeTo, address indexed newFranchisedTo);
    event LogWithdrawFees(address indexed feeTo, address indexed franchisedTo, uint256 sushiFraction, uint256 franchisedFraction);
    event LogFranchisedShare(uint256 franchisedShare);

    // Immutables (for MasterContract and all clones)
    IBentoBoxV1 public immutable bentoBox;
    FranchisedKashiPair public immutable masterContract;

    // MasterContract variables
    address public feeTo;
    address public franchiseTo;
    ...
    uint256 public FRANCHISED_SHARE;
    uint256 private constant FRANCHISED_SHARE_DIVISOR = 1e5;
    ...
    function init(bytes calldata data) public payable override {
        ...
        (collateral, asset, oracle, oracleData, FRANCHISED_SHARE, COLLATERIZATION_RATE, LIQUIDATION_MULTIPLIER) = abi.decode(data, (IERC20, IERC20, IOracle, bytes, uint256, uint256, uint256));
    ...
    modifier solvent() {
        _;
        require(_isSolvent(msg.sender, exchangeRate), "KashiPair: user insolvent");
    }
    ...
    function withdrawFees() public {
        ...
        address _franchiseTo = masterContract.franchiseTo();
        uint256 _feesEarnedFraction = accrueInfo.feesEarnedFraction;
        uint256 sushiFraction = _feesEarnedFraction.mul(FRANCHISED_SHARE) / FRANCHISED_SHARE_DIVISOR;
        uint256 franchisedFraction = _feesEarnedFraction.sub(sushiFraction);
        balanceOf[_feeTo] = balanceOf[_feeTo].add(sushiFraction);
        balanceOf[_franchiseTo] = balanceOf[_franchiseTo].add(franchisedFraction);
        emit Transfer(address(0), _feeTo, sushiFraction);
        emit Transfer(address(0), _franchiseTo, franchisedFraction);
        accrueInfo.feesEarnedFraction = 0;

        emit LogWithdrawFees(_feeTo, _franchiseTo, sushiFraction, franchisedFraction);
    }

    function changeFranchiseShare(uint256 newFranchiseShare) public onlyOwner {
        FRANCHISED_SHARE = newFranchiseShare;
        emit LogFranchisedShare(FRANCHISED_SHARE);
    }

    /// @notice Sets the beneficiary of fees accrued in liquidations.
    /// MasterContract Only Admin function.
    /// @param newFeeTo The address of the receiver.
    /// @param newFranchiseTo The address of the receiver.
    function setFeeTo(address newFeeTo, address newFranchiseTo) public onlyOwner {
        feeTo = newFeeTo;
        franchiseTo = newFranchiseTo;
        emit LogFeeTo(newFeeTo, newFranchiseTo);
    }
}
